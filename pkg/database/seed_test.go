...
func TestSeed(t *testing.T) {
	...
	vmServiceType, err := serviceTypeRepo.Get(ctx, vmServiceTypeID)
	...
	dummyAgentType, err := agentTypeRepo.Get(ctx, dummyAgentTypeID)
	...
	for i, id := range metricTypeIDs {
		...
		mt, err := metricTypeRepo.Get(ctx, id)
	...
	adminToken, err := tokenRepo.Get(ctx, adminTokenID)
	...
	assert.Equal(t, auth.RoleAdmin, adminToken.Role, "Admin token should have admin role")
