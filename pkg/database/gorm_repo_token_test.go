...
func TestTokenRepository(t *testing.T) {
	...
	t.Run("create", func(t *testing.T) {
		t.Run("success - admin token", func(t *testing.T) {
			...
			// Setup
			token := createTestToken(t, auth.RoleAdmin, nil)
			...
			// Verify in database
			found, err := repo.Get(ctx, token.ID)
			require.NoError(t, err)
	...
	t.Run("List", func(t *testing.T) {
		t.Run("success - list all", func(t *testing.T) {
			...
			// Setup
			token1 := createTestToken(t, auth.RoleAdmin, nil)
		...
		t.Run("success - list with role filter", func(t *testing.T) {
			...
			require.NoError(t, err)
			for _, item := range result.Items {
				assert.Equal(t, auth.RoleParticipant, item.Role)
			}
		})
	...
	t.Run("Update", func(t *testing.T) {
		t.Run("success", func(t *testing.T) {
			...
			// Read
			token, err := repo.Get(ctx, token.ID)
			require.NoError(t, err)
			...
			// Verify in database
			updated, err := repo.Get(ctx, token.ID)
			require.NoError(t, err)
	...
	t.Run("DeleteByParticipantID for participant", func(t *testing.T) {
		t.Run("success - deletes tokens with matching participant ID", func(t *testing.T) {
			...
			_, err = repo.Get(ctx, participantToken1.ID)
			assert.ErrorAs(t, err, &domain.NotFoundError{}, "Provider token 1 should be deleted")

			_, err = repo.Get(ctx, participantToken2.ID)
	...
	t.Run("AuthScope", func(t *testing.T) {
		t.Run("success - returns correct auth scope", func(t *testing.T) {
			...
			adminToken := createTestToken(t, auth.RoleAdmin, nil)
