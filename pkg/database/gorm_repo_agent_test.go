...
func TestAgentRepository(t *testing.T) {
	...
	t.Run("FindFirstOnlineByServiceType", func(t *testing.T) {
		t.Run("returns the most-recently-seen connected agent", func(t *testing.T) {
			...
			now := time.Now()
			agentOld := createTestAgentWithStatusUpdate(t, participant.ID, agentType1.ID, domain.AgentConnected, now.Add(-10*time.Minute))
			require.NoError(t, agentRepo.Create(ctx, agentOld))
			agentRecent := createTestAgentWithStatusUpdate(t, participant.ID, agentType2.ID, domain.AgentConnected, now.Add(-1*time.Minute))
			require.NoError(t, agentRepo.Create(ctx, agentRecent))
			// Disconnected agent supporting the same type must be excluded.
