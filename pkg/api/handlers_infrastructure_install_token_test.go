...
func TestBuildInfrastructureInstallURL(t *testing.T) {
	...
	for _, tc := range tests {
		got := buildInstallURL(infrastructureInstallConfigFullPath, tc.base, tc.token)
		if got != tc.want {
...
// route/URL agreement test: chi must resolve what buildInstallURL renders, so
// the URL builder and the route share one template.
func TestInfrastructureInstallSubPath_RouteAndURLAgree(t *testing.T) {
	...
	url := buildInstallURL(infrastructureInstallConfigFullPath, "http://x", "the-token")
