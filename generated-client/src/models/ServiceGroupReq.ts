/* tslint:disable */
/* eslint-disable */
/**
 * Fulcrum Core API
 * A comprehensive cloud infrastructure management system designed to orchestrate and monitor distributed cloud resources across multiple providers.
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface ServiceGroupReq
 */
export interface ServiceGroupReq {
    /**
     * 
     * @type {string}
     * @memberof ServiceGroupReq
     */
    name: string;
    /**
     * 
     * @type {string}
     * @memberof ServiceGroupReq
     */
    consumerId: string;
}

/**
 * Check if a given object implements the ServiceGroupReq interface.
 */
export function instanceOfServiceGroupReq(value: object): value is ServiceGroupReq {
    if (!('name' in value) || value['name'] === undefined) return false;
    if (!('consumerId' in value) || value['consumerId'] === undefined) return false;
    return true;
}

export function ServiceGroupReqFromJSON(json: any): ServiceGroupReq {
    return ServiceGroupReqFromJSONTyped(json, false);
}

export function ServiceGroupReqFromJSONTyped(json: any, ignoreDiscriminator: boolean): ServiceGroupReq {
    if (json == null) {
        return json;
    }
    return {
        
        'name': json['name'],
        'consumerId': json['consumerId'],
    };
}

export function ServiceGroupReqToJSON(json: any): ServiceGroupReq {
    return ServiceGroupReqToJSONTyped(json, false);
}

export function ServiceGroupReqToJSONTyped(value?: ServiceGroupReq | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'name': value['name'],
        'consumerId': value['consumerId'],
    };
}

