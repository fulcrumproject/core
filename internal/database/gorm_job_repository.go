...
var applyJobFilter = mapFilterApplier(map[string]FilterFieldApplier{
	"type":      parserInFilterFieldApplier("type", domain.ParseJobType),
...
func (r *gormJobRepository) CompleteJob(ctx context.Context, jobID domain.UUID, resultData domain.JSON) error {
	now := time.Now()
