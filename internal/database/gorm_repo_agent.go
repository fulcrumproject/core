...
func NewAgentRepository(db *gorm.DB) *GormAgentRepository {
	repo := &GormAgentRepository{
		GormRepository: NewGormRepository[domain.Agent](
			...
			[]string{"Provider", "AgentType", "AgentType.ServiceTypes"}, // Find preload paths
...
func (r *GormAgentRepository) FindByServiceTypeAndTags(ctx context.Context, serviceTypeID domain.UUID, tags []string) ([]*domain.Agent, error) {
...
func (r *GormAgentRepository) AuthScope(ctx context.Context, id domain.UUID) (*domain.AuthTargetScope, error) {
