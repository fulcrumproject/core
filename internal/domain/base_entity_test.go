...
func TestBaseEntity_BeforeCreate(t *testing.T) {
	t.Run("When ID is nil, should generate a new UUID", func(t *testing.T) {
		...
		if uuid.UUID(entity.ID) == uuid.Nil {
			t.Error("BeforeCreate did not generate a new UUID")
		}
	})

	t.Run("When ID is already set, should not change it", func(t *testing.T) {
		// Create a UUID and a BaseEntity with it
		originalID := NewUUID()
		entity := BaseEntity{
			ID:        originalID,
			CreatedAt: time.Now(),
...
func TestBaseEntity_GetID(t *testing.T) {
	...
	t.Run("Should return nil UUID when ID is nil", func(t *testing.T) {
		...
		// Verify nil UUID is returned
		if uuid.UUID(actualID) != uuid.Nil {
			t.Errorf("GetID should return nil UUID, got %v", actualID)
		}
	})
}
