...
func (a Attributes) Validate() error {
	for key, values := range a {
		if key == "" {
			return ErrInvalidAttributes //ValidationError("attribute key cannot be empty")
		}
		if len(values) == 0 {
			return ErrInvalidAttributes //ValidationError("attribute values array cannot be empty")
		}
		for _, value := range values {
			if value == "" {
				return ErrInvalidAttributes //ValidationError("attribute value cannot be empty")
			}
		}
	}
	return nil
