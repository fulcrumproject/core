...
func (h *EventHandler) Routes() func(r chi.Router) {
	return func(r chi.Router) {
		...
		r.With(
			AuthzSimple(domain.SubjectEvent, domain.ActionRead, h.authz),
		).Get("/", h.handleList)

		// Event consumption endpoint with leasing - requires admin role
		r.With(
			AuthzSimple(domain.SubjectEvent, domain.ActionLease, h.authz),
		).Post("/lease", h.handleLease)

		// Event acknowledgement endpoint - requires admin role
		r.With(
			AuthzSimple(domain.SubjectEvent, domain.ActionAck, h.authz),
