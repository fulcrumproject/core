...
func testServiceOption(t *testing.T, env *Env) {
	t.Run("admin creates+gets+updates+deletes scoped to provider", func(t *testing.T) {
		enabled := true
		name := "opt-" + testhelpers.Uniq()
		created := testhelpers.MustPost[api.CreateServiceOptionReq, api.ServiceOptionRes](t, env.AdminClient, "/service-options", api.CreateServiceOptionReq{
			ProviderID:          env.Seed.Provider.ID,
			ServiceOptionTypeID: env.Seed.ServiceOptionType.ID,
			Name:                name,
			Value:               "small",
			Enabled:             &enabled,
		})
		...
		require.Equal(t, env.Seed.ServiceOptionType.ID, created.ServiceOptionTypeID)
		require.True(t, created.Enabled)
		require.NotEqual(t, properties.UUID{}, created.ID)
		require.False(t, time.Time(created.CreatedAt).IsZero())

		got := testhelpers.MustGet[api.ServiceOptionRes](t, env.AdminClient, "/service-options", created.ID)
		...
		newName := "opt-renamed-" + testhelpers.Uniq()
		updated := testhelpers.MustPatch[api.UpdateServiceOptionReq, api.ServiceOptionRes](t, env.AdminClient, "/service-options", created.ID, api.UpdateServiceOptionReq{Name: &newName})
		...
		page := testhelpers.MustList[api.ServiceOptionRes](t, env.AdminClient, "/service-options")
		require.True(t, testhelpers.ContainsID(page.Items, created.ID), "list must include just-created option")

		testhelpers.MustDelete(t, env.AdminClient, "/service-options", created.ID)
		testhelpers.AssertGone(t, env.AdminClient, "/service-options", created.ID)
	})
